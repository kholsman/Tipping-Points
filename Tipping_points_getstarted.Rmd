...
```{r threshold_STEP0,eval=T,include=F,echo=T, results='hide',message=FALSE}
...
```{r threshold_STEP1,eval=T,include=T,echo=T, results='hide',message=FALSE}
  ...
  #------------------------------------
  tmp_gam   <-  gam(delta_var_prcnt ~ s(driver,k=t_knots,bs="tp"),data = datIN)
  ...
  dd        <-  datIN%>%mutate(driver = round(TempC,2) )%>%select(driver, delta_var_prcnt)
...
```{r threshold_STEP2,eval=T,include=T,echo=T, results='hide',message=FALSE}
  ...
  for(int in 1:boot_n){
    ...
    tmpgam        <- gam(delta_var_prcnt~s(driver,k=knotsIN,bs="tp"),data = bootd)
